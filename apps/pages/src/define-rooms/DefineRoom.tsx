/** @jsxImportSource @define-rooms/lib */
...
export type ToolType = "brush" | "eraser" | "lasso" | "magnetic" | "wand" | "magnify" | "move";

type Point = { x: number; y: number };

export type DefineRoomSerializedRoom = {
  id: string;
  name: string;
  description: string;
  tags: string;
...
type Room = DefineRoomSerializedRoom & {
...
export class DefineRoom {
  private readonly options: DefineRoomOptions;
  ...
  public open(image: HTMLImageElement, rooms?: DefineRoomSerializedRoom[]): void {
  ...
  public loadRooms(rooms: DefineRoomSerializedRoom[]): void {
    if (!this.imageData) {
      return;
    }
    ...
    this.rooms = rooms.map((room, index) => {
      ...
      return {
        ...room,
        isConfirmed: room.isConfirmed ?? true,
    ...
    this.rooms.forEach((room) => this.initializeRoomHistory(room.id));
  ...
  private emitRoomsChange(): void {
    if (!this.options.onRoomsChange) {
      return;
    }
    const snapshot = this.rooms.map((room) => ({
      id: room.id,
      name: room.name,
    ...
    this.options.onRoomsChange(snapshot);
  }
