...
const AuthPanel: React.FC<AuthPanelProps> = ({ onAuthenticate, className, variant = 'default' }) => {
  ...
  const badgeText = mode === 'login' ? 'Return to the light' : 'Kindle a DM profile';
  ...
  const toggleHelper = mode === 'login' ? 'Kindle one instead' : 'Use your existing login';
    ...
    <section className={containerClasses} aria-labelledby={`${formId}-title`}>
      ...
      <div className="relative space-y-8">
        <header className="space-y-3">
          <span className="inline-flex items-center rounded-full border border-amber-500/40 bg-amber-100/60 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.3em] text-amber-700 shadow-sm dark:border-amber-400/50 dark:bg-amber-500/10 dark:text-amber-200">
            {badgeText}
          </span>
