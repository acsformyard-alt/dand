...
const AuthPanel: React.FC<AuthPanelProps> = ({ onAuthenticate, className, variant = 'default' }) => {
  ...
  const badgeText = mode === 'login' ? 'Return to the table' : 'Create a TableTorch profile';
    ...
    <section className={containerClasses} aria-labelledby={`${formId}-title`}>
      ...
      <div className="relative space-y-8">
        <header className="space-y-3">
          <span className="inline-flex items-center rounded-full border border-amber-600/40 bg-amber-100/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.3em] text-amber-800 shadow-sm dark:border-amber-400/50 dark:bg-amber-500/10 dark:text-amber-200">
            {badgeText}
          </span>
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <button
              ...
              className="inline-flex items-center rounded-full border border-slate-300/70 bg-white/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-slate-600 transition hover:border-amber-500/60 hover:text-amber-600 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-300 dark:hover:border-amber-400/50 dark:hover:text-amber-200"
              aria-pressed={mode === 'signup'}
            >
              {toggleLabel}
            </button>
          </div>
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">{toggleHelper}</p>
        </header>
        <form
          id={formId}
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 px-5 py-3 text-sm font-semibold uppercase tracking-[0.4em] text-white shadow-lg shadow-orange-500/30 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-amber-500 disabled:cursor-wait disabled:opacity-80"
          >
            {submitLabel}
          </button>
        </form>
