...
const rgbaToCss = (value: RGBAColor) => {
...
const createTorchStage = (canvasHost: HTMLElement): TorchStage | null => {
  ...
  const resolveBackgroundColor = (element: Nullable<HTMLElement>): BackgroundResolution => {
    ...
    let firstColor: RGBAColor | null = null;

    while (node && !visited.has(node)) {
      visited.add(node);
      const style = window.getComputedStyle(node);
      let parsed = parseCssColor(style?.backgroundColor);
      if (!parsed || parsed.a <= 0) {
        parsed = parseCssColor(style?.background);
      }
      if (parsed && parsed.a > 0) {
        if (!firstColor) {
          firstColor = parsed;
    ...
    const nearest = firstColor ?? accumulated;

    return {
      composite: accumulated,
      css: rgbaToCss(nearest),
      nearest,
    };
  };

  const backgroundResolution = resolveBackgroundColor(canvasHost);
  const backgroundColor = rgbaToNumber(backgroundResolution.composite);
  const nearestBackgroundColor = rgbaToNumber(backgroundResolution.nearest);
  const nearestBackgroundAlpha = Math.max(0, Math.min(1, backgroundResolution.nearest.a));
  const backgroundCss = backgroundResolution.css;
  ...
  class Fire {
    ...
    constructor(color: number, app: any, pixelate = false) {
      ...
      cutoutCircle.beginFill(nearestBackgroundColor, nearestBackgroundAlpha);
    ...
    private addCutout(left: boolean) {
      ...
      blob.tint = nearestBackgroundColor;
