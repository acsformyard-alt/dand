...
const rgbaToCssColor = (value: RGBAColor) => {
  const r = clamp255(value.r);
  const g = clamp255(value.g);
  const b = clamp255(value.b);
  const alpha = Math.max(0, Math.min(1, value.a));
  if (alpha >= 0.999) {
    return `rgb(${r} ${g} ${b})`;
  }
  const alphaRounded = Math.round(alpha * 1000) / 1000;
  return `rgb(${r} ${g} ${b} / ${alphaRounded})`;
};
...
const createTorchStage = (canvasHost: HTMLElement): TorchStage | null => {
  ...
  const resolveBackgroundColor = (
  ...
  ): { layered: RGBAColor; flattened: RGBAColor; number: number; layeredCss: string } => {
    ...
    let accumulated: Nullable<RGBAColor> = null;
    ...
    while (node && !visited.has(node)) {
      ...
      if (parsed) {
        accumulated = accumulated ? compositeOver(accumulated, parsed) : parsed;
    ...
    const layered = accumulated ?? defaultColor;
    const flattened = compositeOver(layered, defaultColor);
    return {
      layered,
      flattened,
      number: rgbaToNumber(flattened),
      layeredCss: rgbaToCssColor(layered),
    };
  };

  const backgroundColorInfo = resolveBackgroundColor(canvasHost);
  const backgroundColor = backgroundColorInfo.number;
  const layeredColor = backgroundColorInfo.layered;
  const layeredColorNumber = rgbaToNumber(layeredColor);
  const layeredAlphaRaw = Number.isFinite(layeredColor.a) ? layeredColor.a : 1;
  const layeredAlpha = layeredAlphaRaw > 0 ? Math.max(0.001, Math.min(1, layeredAlphaRaw)) : 1;
  ...
  class Fire {
    ...
    constructor(color: number, app: any, pixelate = false) {
      ...
      cutoutCircle.beginFill(layeredColorNumber, layeredAlpha);
  ...
  class Stage {
    ...
    constructor(canvas: HTMLElement, pixelate = false) {
      ...
      const layeredCss = backgroundColorInfo.layeredCss;
      this.app.renderer.backgroundColor = backgroundColor;
      this.app.view.style.backgroundColor = 'transparent';
      this.app.view.style.backgroundImage = 'none';
      this.host.style.backgroundColor = layeredCss;
