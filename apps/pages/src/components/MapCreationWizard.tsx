...
const MapCreationWizard: React.FC<MapCreationWizardProps> = ({
  ...
  const defineRoomOriginalCloseRef = useRef<(() => void) | null>(null);
  ...
  const handleDefineRoomContainerRef = useCallback(
    (node: HTMLDivElement | null) => {
      if (!node) {
        return;
      }

      let editor = defineRoomRef.current;
      if (!editor) {
        editor = new DefineRoom();
        editor.mount(node);
        editor.element.classList.add('define-room-inline');
        defineRoomOriginalCloseRef.current = editor.close.bind(editor);
        (editor as unknown as { close: () => void }).close = () => {
          syncRoomsFromEditor();
          defineRoomOriginalCloseRef.current?.();
        };
        defineRoomRef.current = editor;
        setDefineRoomReady(true);
      } else {
        editor.element.classList.add('define-room-inline');
        if (editor.element.parentElement !== node) {
          node.appendChild(editor.element);
        }
      }
    },
    [syncRoomsFromEditor],
  ...
  useEffect(() => {
    return () => {
      ...
      if (!editor) return;
      if (defineRoomOriginalCloseRef.current) {
        (editor as unknown as { close: () => void }).close =
          defineRoomOriginalCloseRef.current;
      }
      editor.destroy();
          ...
          {step === 2 && (
            <div className="flex h-full flex-col gap-6">
              <div className="rounded-3xl border border-slate-800/70 bg-slate-900/70 p-6">
                ...
                <h3 className="mt-2 text-xl font-semibold text-white">Outline the spaces on your map</h3>
                <p className="mt-3 text-sm text-slate-400">
                  Paint or lasso the areas that represent distinct rooms, add descriptions, and choose whether they begin
                  revealed. Your edits save automatically when you continue to the next step.
                </p>
              </div>
              <div className="flex-1 min-h-0">
                <div className="relative flex h-full min-h-[520px] flex-col overflow-hidden rounded-3xl border border-slate-800/70 bg-slate-950/70">
                  <div ref={handleDefineRoomContainerRef} className="absolute inset-0" />
                  {(!previewUrl || !defineRoomReady) && (
                    <div className="pointer-events-none absolute inset-0 flex items-center justify-center px-6 text-center text-sm text-slate-400">
                      {previewUrl ? 'Preparing the room editor…' : 'Upload a map image in the first step to begin defining rooms.'}
                    </div>
                  )}
                </div>
              </div>
