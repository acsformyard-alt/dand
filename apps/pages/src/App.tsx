...
const App: React.FC = () => {
  ...
  const navButtonClasses = (view: 'join' | 'manage' | 'create' | 'admin') =>
    `group flex w-full items-center justify-between rounded-2xl border px-5 py-4 text-left text-sm font-semibold uppercase tracking-[0.3em] transition ${
      activeView === view
        ? 'border-amber-400 bg-amber-400/90 text-stone-900 shadow-lg shadow-amber-500/40'
        : 'border-amber-900/40 bg-stone-900/70 text-amber-200 hover:border-amber-400/60 hover:bg-stone-900/80'
    }`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-950/90 via-stone-950/90 to-stone-900/95 text-amber-100">
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-4 py-8 md:flex-row md:py-12">
        <aside className="flex flex-col gap-6 rounded-3xl border border-amber-900/40 bg-stone-950/60 p-6 shadow-2xl shadow-black/40 md:w-72 backdrop-blur">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-amber-300/80">Mission Control</p>
            <h2 className="mt-3 text-2xl font-black uppercase tracking-wide text-amber-400">Command Deck</h2>
          </div>
          <div className="rounded-2xl border border-amber-900/40 bg-stone-900/70 p-4">
            <p className="text-xs uppercase tracking-[0.5em] text-amber-300/70">Logged in</p>
            <p className="mt-2 text-lg font-semibold text-amber-100">{user.displayName}</p>
            <p className="text-xs text-amber-300/60">Torchbearer</p>
          </div>
          <nav className="space-y-3">
            <button className={navButtonClasses('join')} onClick={() => setActiveView('join')}>
              <span>Join Campaign</span>
              <span className="text-[10px] tracking-[0.4em] text-amber-900/80 transition group-hover:text-amber-900">START</span>
            </button>
            <button className={navButtonClasses('manage')} onClick={() => setActiveView('manage')}>
              <span>Manage Campaigns</span>
              <span className="text-[10px] tracking-[0.4em] text-amber-900/80 transition group-hover:text-amber-900">LEDGER</span>
            </button>
            <button className={navButtonClasses('create')} onClick={() => setActiveView('create')}>
              <span>Create Campaign</span>
              <span className="text-[10px] tracking-[0.4em] text-amber-900/80 transition group-hover:text-amber-900">NEW</span>
            </button>
          </nav>
          <div className="mt-auto space-y-2 text-xs text-amber-200/70">
            <p>Need a room code? Ask your DM to share their campaign key.</p>
            <p>Switch tabs to manage, create, or join adventures.</p>
          </div>
        </aside>
        <section className="flex-1 space-y-6">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-amber-900/40 bg-stone-950/60 px-6 py-4 shadow-xl shadow-black/40 backdrop-blur">
            <div>
              <p className="text-xs uppercase tracking-[0.5em] text-amber-300/80">Campaign Control</p>
              <h1 className="text-3xl font-black uppercase tracking-wide text-amber-50">TableTorch</h1>
            </div>
          ...
          {statusMessage && (
            <div className="rounded-3xl border border-amber-500/40 bg-amber-500/10 px-5 py-3 text-sm text-amber-100 shadow-lg shadow-amber-500/10">
              {statusMessage}
            </div>
          )}
          <div className="flex-1 rounded-3xl border border-amber-900/40 bg-stone-950/60 p-6 shadow-2xl shadow-black/40 backdrop-blur">
            {activeView === 'join' && (
              <div className="space-y-6">
                <h2 className="text-3xl font-black uppercase tracking-wide text-amber-300">Join Campaign</h2>
                ...
                <form onSubmit={handleJoinByKey} className="space-y-4">
                  <label className="block text-xs uppercase tracking-[0.4em] text-amber-300/70">
                    Campaign Key
                    <div className="mt-2 flex flex-col gap-3 sm:flex-row">
                      <input
                        ...
                        className="flex-1 rounded-xl border border-amber-900/40 bg-stone-900/70 px-4 py-3 text-sm uppercase tracking-[0.3em] text-amber-100 placeholder:text-amber-300/50 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-400/40"
                      />
                      <button
                        type="submit"
                        className="rounded-xl border border-amber-400/60 bg-amber-500/80 px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-stone-900 transition hover:bg-amber-400/90 focus:outline-none focus:ring-2 focus:ring-amber-400/40"
                      >
                        Join Room
                      </button>
                    </div>
                ...
                <div className="rounded-2xl border border-amber-900/40 bg-stone-900/70 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <h3 className="text-xs uppercase tracking-[0.4em] text-amber-300/70">Active Rooms</h3>
                    <button
                      type="button"
                  ...
                  <ul className="max-h-48 space-y-2 overflow-y-auto pr-1 text-sm">
                    {lobbySessions.map((session) => (
                      <li key={session.id} className="rounded-xl border border-amber-900/40 bg-stone-950/60 p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            <p className="font-semibold text-amber-100">{session.name}</p>
                            <p className="text-[10px] uppercase tracking-[0.4em] text-amber-300/60">Key: {session.id}</p>
                          </div>
                          <button
                            type="button"
                            className="rounded-lg border border-amber-400/60 bg-amber-500/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-stone-900 transition hover:bg-amber-400/90"
                    ...
                    {lobbySessions.length === 0 && (
                      <li className="rounded-xl border border-dashed border-amber-900/40 px-3 py-6 text-center text-xs text-amber-200/60">
                        No active rooms yet.
                      </li>
                    )}
                  </ul>
            ...
            {activeView === 'manage' && (
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h2 className="text-3xl font-black uppercase tracking-wide text-amber-300">Manage Campaigns</h2>
                    <p className="text-sm text-amber-200/80">Select a campaign to open the admin hangar.</p>
                  </div>
                  <button
                    type="button"
                    className="rounded-full border border-amber-400/60 px-5 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-amber-100 transition hover:bg-amber-500/20"
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {campaigns.map((campaign) => (
                    <button
                      key={campaign.id}
                      className="group flex h-full flex-col justify-between rounded-2xl border border-amber-900/40 bg-stone-950/60 p-4 text-left transition hover:border-amber-400/60 hover:bg-stone-900/70"
                      onClick={() => handleOpenCampaignAdmin(campaign)}
                    >
                      <div>
                        <p className="text-xs uppercase tracking-[0.4em] text-amber-300/60">Campaign</p>
                        <h3 className="mt-2 text-lg font-semibold text-amber-50">{campaign.name}</h3>
                        <p className="mt-2 text-xs text-amber-300/70">{campaign.description || 'No description provided.'}</p>
                      </div>
                  ...
                  {campaigns.length === 0 && (
                    <div className="rounded-2xl border border-dashed border-amber-900/30 p-6 text-center text-sm text-amber-300/70">
            ...
            {activeView === 'create' && (
              <div className="space-y-6">
                <h2 className="text-3xl font-black uppercase tracking-wide text-amber-300">Create Campaign</h2>
                <p className="max-w-xl text-sm text-amber-200/80">Set up a new campaign for your players and start building encounters.</p>
                <form onSubmit={handleCreateCampaign} className="space-y-5">
                  <div>
                    <label className="text-xs uppercase tracking-[0.4em] text-amber-300/70">Campaign Name</label>
                    <input
                      value={newCampaignName}
                      onChange={(event) => setNewCampaignName(event.target.value)}
                      className="mt-2 w-full rounded-xl border border-amber-900/40 bg-stone-950/70 px-4 py-3 text-sm text-amber-100 placeholder:text-amber-300/60 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-400/40"
                      placeholder="Give your mission a title"
                    />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-[0.4em] text-amber-300/70">Description</label>
                    <textarea
                  ...
                  <label className="flex items-center gap-3 text-xs uppercase tracking-[0.4em] text-amber-300/70">
                    <input
                      type="checkbox"
                      checked={newCampaignPublic}
                      onChange={(event) => setNewCampaignPublic(event.target.checked)}
                      className="h-4 w-4 rounded border border-amber-900/40 bg-stone-900 text-amber-400 focus:ring-amber-400"
                    />
                    <span className="text-amber-200/80">List publicly for players to discover</span>
                  </label>
                  {createError && <p className="text-xs font-semibold text-rose-300">{createError}</p>}
                  <div className="flex flex-wrap gap-3">
                    <button
                      type="submit"
                      className="rounded-xl border border-amber-400/60 bg-amber-500/80 px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-stone-900 transition hover:bg-amber-400/90"
                    >
                      Launch Campaign
                    </button>
                    <button
                      type="button"
                      className="rounded-xl border border-amber-900/30 px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-amber-200/80 transition hover:border-amber-400/60 hover:text-amber-100"
                      onClick={() => {
                        setNewCampaignName('');
                        setNewCampaignDescription('');
                        setNewCampaignPublic(false);
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="text-3xl font-black uppercase tracking-wide text-amber-50">{selectedCampaign.name}</h2>
                    {selectedCampaign.description && <p className="text-sm text-amber-200/80">{selectedCampaign.description}</p>}
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      type="button"
                      className="rounded-full border border-amber-900/30 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-amber-200/80 transition hover:border-amber-400/60 hover:text-amber-100"
                    ...
                    <button
                      type="button"
                      className="rounded-full border border-amber-400/60 bg-amber-500/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-stone-900 transition hover:bg-amber-400/90"
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
                    <div className="space-y-6">
                      {selectedMap ? (
                        <div className="rounded-2xl border border-amber-900/40 bg-stone-900/70 p-6">
                          <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
                            <div>
                              <h3 className="text-2xl font-semibold text-amber-50">{selectedMap.name}</h3>
                              {mapDescription && <p className="mt-2 text-sm text-amber-200/80">{mapDescription}</p>}
                              {!mapDescription && mapNotes && <p className="mt-2 text-sm text-amber-300/70">{mapNotes}</p>}
                            </div>
                            <div className="flex flex-wrap items-center gap-4 text-xs uppercase tracking-[0.4em] text-amber-300/60">
                          ...
                          <div className="mt-6 grid gap-4 lg:grid-cols-2">
                            <div className="rounded-2xl border border-amber-900/40 bg-stone-950/70 p-4">
                              <p className="text-[10px] uppercase tracking-[0.4em] text-amber-300/60">Grouping</p>
                              <p className="mt-2 text-sm font-semibold text-amber-200">{mapGrouping}</p>
                              {mapNotes && mapDescription && (
                                <p className="mt-3 text-xs text-amber-300/70">Notes: {mapNotes}</p>
                              )}
                            </div>
                            <div className="rounded-2xl border border-amber-900/40 bg-stone-950/70 p-4">
                              <p className="text-[10px] uppercase tracking-[0.4em] text-amber-300/60">Tags</p>
                              {mapTags.length > 0 ? (
                                <div className="mt-3 flex flex-wrap gap-2">
                                  {mapTags.map((tag) => (
                                    <span
                                      key={tag}
                                      className="inline-flex items-center rounded-full border border-amber-900/30 bg-stone-900/70 px-2 py-1 text-[10px] uppercase tracking-[0.3em] text-amber-200/80"
                                    >
                                      {tag}
                                    </span>
                                  ))}
                                ...
                                <p className="mt-3 text-xs text-amber-300/60">No tags assigned yet.</p>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="rounded-2xl border border-dashed border-amber-900/30 p-12 text-center text-sm text-amber-300/70">
                          Select or create a map to begin shaping your encounter.
                        </div>
                      )}
                    </div>
                    <div className="space-y-6">
                      <div className="rounded-2xl border border-amber-900/40 bg-stone-900/70 p-5">
                        <div className="mb-3 flex items-center justify-between">
                          <h3 className="text-xs uppercase tracking-[0.4em] text-amber-300/60">My Sessions</h3>
                        </div>
                        <ul className="space-y-2 text-sm">
                          {mySessions.map((session) => (
                            <li key={session.id}>
                              <button
                                onClick={() => handleJoinSession(session)}
                                className="w-full rounded-xl border border-amber-900/40 bg-stone-950/60 px-3 py-2 text-left text-amber-200/80 transition hover:border-amber-400/60 hover:text-amber-100"
                              >
                                {session.name}
                              </button>
                            </li>
                          ...
                          {mySessions.length === 0 && (
                            <li className="rounded-xl border border-dashed border-amber-900/30 px-3 py-6 text-center text-xs text-amber-300/60">
                              No active sessions.
                            </li>
                          )}
                        </ul>
                      ...
                      <div className="rounded-2xl border border-amber-900/40 bg-stone-900/70 p-5">
                        <div className="mb-3 flex items-center justify-between">
                          <h3 className="text-xs uppercase tracking-[0.4em] text-amber-300/60">Lobby</h3>
                          <button
                            type="button"
                            className="text-xs font-semibold uppercase tracking-[0.3em] text-amber-200 transition hover:text-amber-100"
                            onClick={refreshLobby}
                        ...
                        <ul className="space-y-2 text-sm">
                          {lobbySessions.map((session) => (
                            <li key={session.id} className="rounded-xl border border-amber-900/40 bg-stone-950/60 p-3">
                              <p className="font-semibold text-amber-100">{session.name}</p>
                              <p className="text-[10px] uppercase tracking-[0.4em] text-amber-300/60">Campaign: {session.campaignName ?? 'Unknown'}</p>
                              <p className="text-[10px] uppercase tracking-[0.4em] text-amber-300/60">Map: {session.mapName ?? 'Unknown'}</p>
                              <button
                                type="button"
                                className="mt-3 w-full rounded-xl border border-amber-400/60 bg-amber-500/80 px-3 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-stone-900 transition hover:bg-amber-400/90"
                                onClick={() => handleJoinSession(session)}
                              >
