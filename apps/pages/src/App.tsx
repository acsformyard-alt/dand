...
const App: React.FC = () => {
  ...
  const navButtonClasses = (view: 'join' | 'manage' | 'create' | 'admin') =>
    `group flex w-full items-center justify-between rounded-2xl border px-5 py-4 text-left text-sm font-semibold uppercase tracking-[0.3em] transition ${
      activeView === view
        ...
        : 'border-slate-800/70 bg-slate-900/60 text-slate-300 hover:border-amber-400/60 hover:bg-slate-800/80 hover:text-amber-200'
    }`;
  ...
  if (activeSession) {
    return (
      <div className="min-h-screen bg-amber-50 p-6 dark:bg-slate-900">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              className="rounded-full border border-amber-300 px-3 py-1 text-xs text-amber-700 transition hover:bg-amber-100 dark:border-amber-500/40 dark:text-amber-200 dark:hover:bg-slate-800"
        ...
        {statusMessage && (
          <div className="mb-4 rounded border border-amber-200 bg-white px-4 py-2 text-sm text-slate-600 shadow dark:border-amber-500/40 dark:bg-slate-800 dark:text-slate-200">
            {statusMessage}
          </div>
        )}
        <SessionViewer
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#150d06] via-slate-950 to-[#090807] text-slate-100">
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-4 py-8 md:flex-row md:py-12">
        <aside className="flex flex-col gap-6 rounded-3xl border border-amber-500/30 bg-slate-950/70 p-6 shadow-2xl shadow-amber-500/10 md:w-72">
          <div>
          ...
          <div className="rounded-2xl border border-amber-500/20 bg-slate-900/70 p-4">
            ...
            <p className="text-xs text-slate-500">Lantern lit</p>
          </div>
          <nav className="space-y-3">
            <button className={navButtonClasses('join')} onClick={() => setActiveView('join')}>
              <span>Join Campaign</span>
              <span className="text-[10px] tracking-[0.4em] text-slate-900/70 transition group-hover:text-slate-900/90">GLOW</span>
            </button>
            <button className={navButtonClasses('manage')} onClick={() => setActiveView('manage')}>
              <span>Manage Campaigns</span>
              <span className="text-[10px] tracking-[0.4em] text-slate-900/70 transition group-hover:text-slate-900/90">TEND</span>
            </button>
            <button className={navButtonClasses('create')} onClick={() => setActiveView('create')}>
              <span>Create Campaign</span>
              <span className="text-[10px] tracking-[0.4em] text-slate-900/70 transition group-hover:text-slate-900/90">SPARK</span>
            </button>
          </nav>
          <div className="mt-auto space-y-2 text-xs text-slate-500">
            <p>Need a room code? Ask your DM to pass along their TableTorch key.</p>
            <p>Use the tabs to tend the flame, prepare new scenes, or join the glow.</p>
          </div>
        </aside>
        <section className="flex-1 space-y-6">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-amber-500/30 bg-slate-950/70 px-6 py-4 shadow-xl shadow-amber-500/10">
            <div>
              <p className="text-xs uppercase tracking-[0.5em] text-amber-300">Lantern Control</p>
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-300">Select a campaign to open the keeper's vault.</p>
                  </div>
                  <button
                    type="button"
