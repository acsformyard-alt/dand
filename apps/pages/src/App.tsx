...
import { setParchmentCssVariable } from './theme/parchmentTexture';
import type {
  AuthResponse,
  Campaign,
  LobbySessionSummary,
...
const App: React.FC = () => {
  ...
  const navButtonClasses = (view: 'join' | 'manage' | 'create' | 'admin') =>
    `group flex w-full items-center justify-between rounded-2xl border px-5 py-4 text-left text-sm font-semibold uppercase tracking-[0.3em] transition ${
      activeView === view
        ? 'border-amber-500 bg-amber-200/90 text-amber-900 shadow-lg shadow-amber-500/30 dark:bg-amber-400/90 dark:text-slate-900 dark:shadow-amber-500/40'
        : 'border-amber-900/30 bg-white/70 text-slate-700 hover:border-amber-400/60 hover:bg-amber-50/70 dark:border-slate-800/70 dark:bg-slate-900/60 dark:text-slate-200 dark:hover:border-amber-400/60 dark:hover:bg-slate-900/70'
    }`;
  ...
  if (activeSession) {
    return (
      <div className="min-h-screen bg-parchment-app p-6 text-slate-900 transition-colors dark:text-slate-100">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-amber-600 drop-shadow-sm dark:text-amber-300">TableTorch</h1>
            <p className="text-sm text-slate-600 dark:text-slate-400">Logged in as {user.displayName}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              className="rounded-full border border-amber-900/30 bg-white/70 px-3 py-1 text-xs text-slate-700 transition hover:border-amber-400/60 hover:bg-amber-50/70 dark:border-slate-600 dark:bg-slate-900/60 dark:text-slate-100 dark:hover:border-amber-400/60 dark:hover:bg-slate-800"
        ...
        {statusMessage && (
          <div className="mb-4 rounded border border-amber-900/30 bg-white/80 px-4 py-2 text-sm text-slate-700 shadow dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
            {statusMessage}
          </div>
        )}
        <SessionViewer
  ...
  return (
    <div className="min-h-screen bg-parchment-app text-slate-900 transition-colors dark:text-slate-100">
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-4 py-8 md:flex-row md:py-12">
        <aside className="flex flex-col gap-6 rounded-3xl border border-amber-900/25 bg-white/80 p-6 shadow-2xl backdrop-blur md:w-72 dark:border-slate-800/70 dark:bg-slate-950/70">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-amber-900 dark:text-amber-300">Mission Control</p>
            <h2 className="mt-3 text-2xl font-black uppercase tracking-wide text-amber-600 dark:text-amber-300">Command Deck</h2>
          </div>
          <div className="rounded-2xl border border-amber-900/25 bg-white/70 p-4 dark:border-slate-800/70 dark:bg-slate-900/70">
            <p className="text-xs uppercase tracking-[0.5em] text-amber-800 dark:text-amber-300">Logged in</p>
          ...
          <nav className="space-y-3">
            <button className={navButtonClasses('join')} onClick={() => setActiveView('join')}>
              ...
              <span className="text-[10px] tracking-[0.4em] text-amber-900/70 transition group-hover:text-amber-900 dark:text-slate-900/70 dark:group-hover:text-slate-900/90">START</span>
            </button>
            <button className={navButtonClasses('manage')} onClick={() => setActiveView('manage')}>
              <span>Manage Campaigns</span>
              <span className="text-[10px] tracking-[0.4em] text-amber-900/70 transition group-hover:text-amber-900 dark:text-slate-900/70 dark:group-hover:text-slate-900/90">HANGAR</span>
            </button>
            <button className={navButtonClasses('create')} onClick={() => setActiveView('create')}>
              <span>Create Campaign</span>
              <span className="text-[10px] tracking-[0.4em] text-amber-900/70 transition group-hover:text-amber-900 dark:text-slate-900/70 dark:group-hover:text-slate-900/90">NEW</span>
            </button>
          </nav>
        ...
        <section className="flex-1 space-y-6">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-amber-900/25 bg-white/80 px-6 py-4 shadow-xl backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
            <div>
              <p className="text-xs uppercase tracking-[0.5em] text-amber-800 dark:text-amber-300">Campaign Control</p>
              <h1 className="text-3xl font-black uppercase tracking-wide text-slate-900 dark:text-white">TableTorch</h1>
            </div>
            <div className="flex items-center gap-3">
              <button
                className="rounded-full border border-amber-900/30 bg-white/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-amber-800 transition hover:border-amber-400/60 hover:bg-amber-50/70 dark:border-amber-400/60 dark:bg-amber-400/10 dark:text-amber-200 dark:hover:bg-amber-400/20"
                onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              >
                {themeLabel}
              </button>
              <button
                className="rounded-full border border-rose-400/60 bg-rose-100/60 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-rose-700 transition hover:bg-rose-200/70 dark:bg-rose-500/20 dark:text-rose-200 dark:hover:bg-rose-500/30"
                onClick={handleLogout}
              >
                Logout
              </button>
          ...
          {statusMessage && (
            <div className="rounded-3xl border border-amber-900/30 bg-amber-50/80 px-5 py-3 text-sm text-amber-800 shadow-lg shadow-amber-200/40 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
              {statusMessage}
            </div>
          )}
          <div className="flex-1 rounded-3xl border border-amber-900/25 bg-white/80 p-6 shadow-2xl backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
            {activeView === 'join' && (
              <div className="space-y-6">
                ...
                <form onSubmit={handleJoinByKey} className="space-y-4">
                  <label className="block text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-400">
                    Campaign Key
                ...
                <div className="rounded-2xl border border-amber-900/25 bg-white/70 p-4 dark:border-slate-800/70 dark:bg-slate-900/70">
                  ...
                  <ul className="max-h-48 space-y-2 overflow-y-auto pr-1 text-sm">
                    {lobbySessions.map((session) => (
                      <li key={session.id} className="rounded-xl border border-amber-900/25 bg-white/80 p-3 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/60">
                    ...
                    {lobbySessions.length === 0 && (
                      <li className="rounded-xl border border-dashed border-amber-900/25 px-3 py-6 text-center text-xs text-slate-500 dark:border-slate-700/70">
                        No active rooms yet.
                      </li>
                    )}
                  </ul>
            ...
            {activeView === 'manage' && (
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    className="rounded-full border border-amber-900/30 px-5 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-amber-700 transition hover:border-amber-400/60 hover:bg-amber-50/70 dark:border-amber-400/60 dark:text-amber-200 dark:hover:bg-amber-500/20"
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {campaigns.map((campaign) => (
                    <button
                      key={campaign.id}
                      className="group flex h-full flex-col justify-between rounded-2xl border border-amber-900/25 bg-white/80 p-4 text-left transition hover:border-amber-400/60 hover:bg-amber-50/80 dark:border-slate-800/70 dark:bg-slate-950/60 dark:hover:bg-slate-900/70"
                      onClick={() => handleOpenCampaignAdmin(campaign)}
                    >
                      <div>
                        <p className="text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-500">Campaign</p>
            ...
            {activeView === 'create' && (
              <div className="space-y-6">
                ...
                <form onSubmit={handleCreateCampaign} className="space-y-5">
                  <div>
                    <label className="text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-400">Campaign Name</label>
                    <input
                      value={newCampaignName}
                      onChange={(event) => setNewCampaignName(event.target.value)}
                      className="mt-2 w-full rounded-xl border border-amber-900/30 bg-white/80 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-500 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-400/40 dark:border-slate-800/60 dark:bg-slate-950/70 dark:text-slate-100 dark:placeholder:text-slate-500"
                      placeholder="Give your mission a title"
                    />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-400">Description</label>
                    <textarea
                  ...
                  <label className="flex items-center gap-3 text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-400">
                    <input
                      type="checkbox"
                      checked={newCampaignPublic}
                      onChange={(event) => setNewCampaignPublic(event.target.checked)}
                      className="h-4 w-4 rounded border border-amber-900/30 bg-white text-amber-500 focus:ring-amber-400 dark:border-slate-700 dark:bg-slate-900"
                    />
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <button
                      type="button"
                      className="rounded-xl border border-amber-900/30 px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-slate-600 transition hover:border-amber-400/60 hover:text-amber-700 dark:border-slate-700/70 dark:text-slate-300 dark:hover:text-amber-200"
                      onClick={() => {
                        setNewCampaignName('');
                        setNewCampaignDescription('');
                        setNewCampaignPublic(false);
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      type="button"
                      className="rounded-full border border-amber-900/30 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-slate-600 transition hover:border-amber-400/60 hover:text-amber-700 dark:border-slate-700/70 dark:text-slate-300 dark:hover:text-amber-200"
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
                    <div className="space-y-6">
                      {selectedMap ? (
                        <div className="rounded-2xl border border-amber-900/25 bg-white/80 p-6 dark:border-slate-800/70 dark:bg-slate-900/70">
                          <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-500">
                          ...
                          <div className="overflow-hidden rounded-2xl border border-amber-900/25 bg-white/90 dark:border-slate-800/70 dark:bg-slate-950/70">
                            <MapMaskCanvas
                          ...
                          <div className="mt-6 grid gap-4 lg:grid-cols-2">
                            <div className="rounded-2xl border border-amber-900/25 bg-white/80 p-4 dark:border-slate-800/70 dark:bg-slate-950/70">
                              <p className="text-[10px] uppercase tracking-[0.4em] text-slate-600 dark:text-slate-500">Grouping</p>
                            ...
                            <div className="rounded-2xl border border-amber-900/25 bg-white/80 p-4 dark:border-slate-800/70 dark:bg-slate-950/70">
                              <p className="text-[10px] uppercase tracking-[0.4em] text-slate-600 dark:text-slate-500">Tags</p>
                              {mapTags.length > 0 ? (
                                <div className="mt-3 flex flex-wrap gap-2">
                                  {mapTags.map((tag) => (
                                    <span
                                      key={tag}
                                      className="inline-flex items-center rounded-full border border-amber-900/30 bg-white/70 px-2 py-1 text-[10px] uppercase tracking-[0.3em] text-slate-700 dark:border-slate-700/70 dark:bg-slate-900/70 dark:text-slate-300"
                                    >
                                      {tag}
                                    </span>
                                  ))}
                    ...
                    <div className="space-y-6">
                      <div className="rounded-2xl border border-amber-900/25 bg-white/80 p-5 dark:border-slate-800/70 dark:bg-slate-900/70">
                        <div className="mb-3 flex items-center justify-between">
                          <h3 className="text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-400">My Sessions</h3>
                        </div>
                        <ul className="space-y-2 text-sm">
                          {mySessions.map((session) => (
                            <li key={session.id}>
                              <button
                                onClick={() => handleJoinSession(session)}
                                className="w-full rounded-xl border border-amber-900/25 bg-white/80 px-3 py-2 text-left text-slate-700 transition hover:border-amber-400/60 hover:text-amber-700 dark:border-slate-800/70 dark:bg-slate-950/60 dark:text-slate-300 dark:hover:text-amber-100"
                              >
                                {session.name}
                              </button>
                            </li>
                      ...
                      <div className="rounded-2xl border border-amber-900/25 bg-white/80 p-5 dark:border-slate-800/70 dark:bg-slate-900/70">
                        <div className="mb-3 flex items-center justify-between">
                          <h3 className="text-xs uppercase tracking-[0.4em] text-slate-600 dark:text-slate-400">Lobby</h3>
                          <button
                            type="button"
                            className="text-xs font-semibold uppercase tracking-[0.3em] text-amber-700 transition hover:text-amber-900 dark:text-amber-200 dark:hover:text-amber-100"
                            onClick={refreshLobby}
